function vclamp(action)
% BME 445/545 - Developed by Dr. Keigo Kawaji kkawaji@iit.edu
% You will need both vcgui.m and vclamp.m to run this code.

global a;

% Below shows how parameters are passed from the 
% Graphical User Interface into this vclamp.m code:
%--------------------------------------------------%
% vclampval = str2num(get(EditHandle1,'String'));  %
% EditHandle6 = findobj(gcbf,'Tag','EditText6');   %
% Nae = str2num(get(EditHandle6,'String'));        %
% ENA = 25*log(Nae/45.0)+60;                       %
%--------------------------------------------------% 

%% Extract parameters from vcgui.m for algorithmic use in this .m file
%++BME445/545 LAB_A MainTask_ - Programming the Output Metadata - 
% Make necessary modification somewhere in this parameter declaration
% below. 2021.02.10

EditHandle1=findobj(gcbf,'Tag','EditText1');
V_clamp1=str2num(get(EditHandle1,'String'));
EditHandle1a=findobj(gcbf,'Tag','EditText1a');
use_modified = get(EditHandle1a,'Visible');
EditHandle2=findobj(gcbf,'Tag','EditText2');
tdel1=str2num(get(EditHandle2,'String'));
EditHandle3=findobj(gcbf,'Tag','EditText3');
tend=str2num(get(EditHandle3,'String'));
...
%%%%%%%%%%%%%%%%%%%%%%%
DT = 0.05;      %time step
T_MAX = tend;   %Maximum Time
t = 0:DT:T_MAX; 

V_REST = 0.0;   %Initial Membrane Voltage

G_NA = 120.0;	% mS/cm^2 % Sodium Conductance
G_K = 25.0;	    % mS/cm^2 % Potassium Conductance
G_LEAK = 0.3;	% mS/cm^2 % Leakage Conductance will not be covered in 445/545.

E_K = -8.0;         % mV  % Nernst Potential Potassium
...
for i=1:length(t)
    ...
    Im(i) = (gNa*(V-E_NA) + gK*(V-E_K));        %Membrane Current 
...
figure(a)
position = [(350 + (80*4))/1000, 0.1, 0.3, 0.8];
...
if displayflag == 1
    plot(ax, t, Im, 'b-');
    axis(ax, [0 T_MAX ymin ymax]);
    title(ax, 'Measured Membrane Current vs Time Plot');
    xlabel(ax, 'time (mSec)');
    ylabel(ax, 'Im (mA/sqcm)');
elseif displayflag == 2
    plot(ax, t, g_K, 'b-');
    axis(ax, [0 T_MAX ymin ymax]);
    title(ax, 'Potassium Conductance vs Time Plot');
    xlabel(ax, 'time (mSec)');
    ylabel(ax, 'g_K ');
elseif displayflag == 3
    plot(ax, t, g_Na, 'b-');
    axis(ax, [0 T_MAX ymin ymax]);
    title(ax, 'Sodium Conductance vs Time Plot');
    xlabel(ax, 'time (mSec)');
    ylabel(ax, 'g_Na ');
end
